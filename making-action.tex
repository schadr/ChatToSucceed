...
Measuring the overlap between social relations, as represented by the communication among developers, and technical relations, as represented by artifact dependencies, socio-technical congruence shows a relation between build success and maximum coverage of technical relatopns by social relations~\cite{}.
...
We and others showed that additionally to the relation with produtivity socio-technical congruence also has an affect on software quality.  

Although socio-technical congruence has the potential to generate recommendations that can be used to prevent failures (as detailed earlier in Section~\cref{} and Chapter~\ref{}), it currently lacks in its actionablity to utelize the insights produces so far.
...
They found that using these Mylyn traces to gighly accurate in approximating the congruence metrics calculated using change-sets.
