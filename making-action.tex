...
This build system allows for continuous testing and thus helps the students to get a better understanding of their progress with respect to implementing their tasks for a given sprint.
...
\item Check if using the social-network helps to finding the right people to include in order resolving the issue.
\end{enumerate}
...
After the issue was resolved the team fixing the issue issued the concern in the following retrospective and sprint planning as well as contacted the respective team.
...
We showed in a classroom setting that based on data that can be gathered in a team setting that recommendations preventing build failures can be generated.
