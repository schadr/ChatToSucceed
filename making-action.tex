...
% generally answer the question why it is ok to do it this way
...
% some closing words towards how many instances we are looking at and why this numner is sufficient 
...
% problems that do matter
% build failures that prevent other teams from continuing their work in the next sprint
%% most of them are merge conflicts that happend at the end of a sprint when integrating code of different teams


% problems that do not matter
% failed test cases
%% product owner approval was of more importance than actual failing test cases
%% further many test cases became out dated over the lifetime of the project

\subsection{Preventable Build Failures}
% the change-sets of the different merge conflicts show a definite overlap
% furthermore the edit and select statements captured by our plug-in also allow to infer the conflict across teams

\subsection{The Right Recommendations}
% looking at the communication going on we definetly see missing links between the change-set owners
% since the resolution of the merge conflict usually involved the person finding the issue to resolve it the social network is of no use
% judging from the description of the issue it could have been prevented if the ppl working on the same code would have known about it at the time of modification
