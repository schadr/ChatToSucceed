...
% merge first three chapters into background
% remove constructs from methodology
% write new conclusion
...
\section{TODO Socio-technical coordination and builds in RTC}
\label{sec:data}
Before we describe our approach to collect data and construct
socio-technical networks in Rational Team Concert, we give a brief
description of the Rational Team Concert team and project. Subsequent,
...
\subsection{Development and Builds in the RTC Team}
The Rational Team Concert team is a large distributed team and uses the Rational Team Concertplatform for
development. The Rational Team Concert development involves distributed collaboration over 16
different sites located in the United States, Canada, and Europe. Seven sites are
active in Rational Team Concert development and testing. There are 151 active contributors
working in 47 teams at these locations, where developers can belong to multiple
teams. Each team is responsible for developing a subsystem or component of Rational Team Concert
...
team.
Furthermore, the Rational Team Concert team's development process demands that the developers coordinate using work item discussion. 

The coordination process within
each iteration requires the integration of subsystems developed by individual
Rational Team Concert teams in a major milestone build of the product.
Each team owns a source code Stream for collaboration and concurrent
implementation of the subsystem. A Stream is the Rational Team Concert equivalent to a branch of a
...
Stream, the team publishes the change sets into the Rational Team Concert Project Integration
Stream. At the project level, the automated
Rational Team Concertintegration builds the subsystems of all teams. 

We mined the Rational Team Concert project repository between April and July 2008, and analyzed a
total of 328 builds, out of which 99 were failed and 227 were successful.
Table~\ref{tab:jazzbuildinfo} contains summary statistics describing the Rational Team Concert
...
\subsection{TODO Extracting Social Networks}
\label{subsec:social}
To model the communication between developers for a given build we construct \emph{social networks}. 
We use the information contained in the Rational Team Concert work items for the construction. 
A \emph{work item} in Rational Team Concert is the basic unit of work. 
It describes a general task which can be, but is not restricted to, a bug fix or feature request.
Developers coordinate about work on work items by posting comments in a discussion board style which we use as conceptualization of their coordination behaviour.
...
build in Rational Team Concert. 
...
the Rational Team Concertrepository.




\subsection{TODO Constructing Socio-Technical Networks}
...
Besides the technical side of the approach to generate statistically relevant recommendations as Murphy et al~\cite{} pointed out, developers need to accept such recommendation for them to be ultimately useful.