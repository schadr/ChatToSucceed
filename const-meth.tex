...
This thesis is aiming to answer two major research questions that we broke down into six research questions that are each covered in one chapter of this thesis (with the exception of \textbf{RQ~2.2} and \textbf{RQ~2.3}).
With respect to methodology these six research questions can be roughly organized into three groups by their respective methodology employed to answer them.

The first three research questions as mentioned below lend themselves to quantitative analysis:
\begin{description}
...
\end{description}
All three questions required the analysis of social networks or socio-technical networks.
Both networks can be generated by mining software repositories, as we were provided with by the IBM Rational Team Concert development team described in the previous Chapter~\ref{chap:rtc}.
Combined with statistical analysis methods, such as hypothesis tests, regression analysis, and prediction analysis, this quantitative analysis method enables us to obtain statistically valid results using state-of-the-art analysis techniques with minimally interfering with the productivity of a developer (which is limited to obtaining access to the information). 

The following two research questions lends themselves to be explored using quantitative analysis techniques.
\begin{description}
...
\end{description}
Since in both cases the recommendations need to be accepted by developers, we conducted an observational study by becoming part of the team and followed up with interviews and questionnaires.
By becoming part of the study we had both the opportunity to immerse ourselves in the terminology and process employed by the development team as well as give back the time as a developer that we later asked back by interviewing developers as well as asking them to fill out questionnaires.
Using grounded theory we extracted insights from our observations and interviews that yield insights into answering the two aforementioned research questions.

The final research questions, although, quantitative in effect distinguishes itself methodologically from the previous question in that we had more control over and access to the study participant and how they worked with source code.
\begin{description}
\item[RQ 2.4] Can recommendations be given in a timely manner?
\end{description}
Essentially we had access to when participants selected and modified source code as well as to diaries each participant kept.
This very fine grained information allowed us to extract problems reported by the participants and using the fine grained recorded information to investigate if we would have been able to generate a recommendation that might have prevented the issue from arising.
