...
%
The motivation behind using a matrix format with the matrix multiplication stems from the creation of the separate matrixes.
...
The matrix multiplication allows to derive the developer interdependencies without requiring direct access to the data.
%
...
Further multiplying this matrix with the transposed task assignment matrix yields a developer by developer matrix that indicates which developers are dependent on each others work through at least one task. 
%
...
The current body of knowledge agrees that lapses in coordination (insufficient processes or communication tools) is a major cause for integration issues.
...
This lead to the study of socio-technical congruence as a measure of productivity. 
...
These social and technical dimensions can be expressed as networks of developer that only differ in their connections which can either be social or technical.
