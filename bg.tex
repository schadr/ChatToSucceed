...
\label{chap:bg}
In this chapter we provide an overview of three areas that are relevant to the research conducted with this thesis: (1) the research on software builds, (2) the research around the concept of socio-technical congruence, (3) failure prediction in software engineering, and (4) recommender systems in software engineering.
...
The issue that shifts into focus once the actual process of creating the build is thoroughly optimized is to gain an idea of whether a build will fail or succeed.
...
Following we review literature with respect to coordination and integration, where builds are one form of integration (Section~\ref{sec:RelatedCommunication}.
We compliment that review with research conducted in the field of predicting failures using communication as one goal in this thesis is to improve communication among software developer with respect to build success a measure of software quality.
...
While technical networks are powerful failure predictors, they are only able to point to source code files that are more likely to fail in the field and thus should be more tests.
...
(1) identify the parts of the social network that need to be changed
and (2) devise strategies to change the social network in a timely fashion.
In case of failure prevention the current way socio-technical congruence is conceptualized might be sufficient in that it is enough to compute after a task has been completed to assess the quality of the submitted code.
...
We start in the second part of this thesis with investigating the influence of communication among team members in the form social networks on build success.
